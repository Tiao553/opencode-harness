...
| # | Decision | Rationale | Status |
|---|----------|-----------|--------|
| D7 | `/workflow:iterate` command for mid-stream changes | Maintains traceability | **Implemented** |
...
+------------------+         |          DESIGN              |
                             |                              |
                             |  Agent Matching:             |
                             |  Glob(~/.config/opencode/agents/*.agent.md) |
                             |         |                    |
                             |         v                    |
                             |  +--------------------+      |
                             |  | Capability Index   |      |
...
|-------|---------|-------|-------|--------|--------------|
| 0 | `/workflow:brainstorm` | Opus | Vague idea | BRAINSTORM_*.md | Max 5 questions, 3 approaches |
| 1 | `/workflow:define` | Opus | Requirements | DEFINE_*.md | Clarity Score >= 12/15 |
| 2 | `/workflow:design` | Opus | DEFINE doc | DESIGN_*.md | All files have agents |
| 3 | `/workflow:build` | Sonnet | DESIGN doc | Code + BUILD_REPORT | Tests pass |
| 3.5 | `/workflow:validate` | Sonnet | DEFINE + DESIGN + BUILD_REPORT + code | VALIDATION_REPORT + RUNBOOK/ROADMAP | Score >= 90 and zero CRITICAL for Ship |
| 4 | `/workflow:ship` | Haiku | Approved validation artifacts | archive/ + SHIPPED_*.md | Lessons captured |
...
**Framework-Agnostic:** New agents added to `~/.config/opencode/agents/` automatically become available for matching -- zero configuration.
...
Domains are registered in `~/.config/opencode/kb/_index.yaml`. Use `/knowledge:create-kb` to add new domains.
...
| `/workflow:design` | 2 | Create architecture + agent matching | Opus |
| `/workflow:build` | 3 | Execute with agent delegation | Sonnet |
| `/workflow:ship` | 4 | Archive with lessons learned | Haiku |
| `/workflow:iterate` | Any | Update documents mid-stream | Sonnet |
...
| `/data:schema` | Interactive schema design (star schema, Data Vault, SCD) |
| `/data:data-quality` | Quality rules generation (Great Expectations, Soda) |
| `/data:lakehouse` | Table format and catalog guidance (Iceberg, Delta) |
| `/data:sql-review` | SQL-specific code review |
| `/data:ai-pipeline` | RAG/embedding pipeline scaffolding |
...
| Command | Purpose |
|---------|---------|
| `/knowledge:create-kb` | Create a complete KB domain from scratch |
| `/review:review` | Dual AI code review |
| `/core:meeting` | Meeting transcript analysis |
| `/core:memory` | Save session insights to storage |
| `/core:sync-context` | Update CLAUDE.md from codebase |
| `/core:readme-maker` | Generate README from codebase analysis |
...
| FR-004 | System must support `/workflow:iterate` for mid-stream changes | P1-High | Implemented |
...
| **Empty KB Domains** | "We don't have patterns" | Use `/knowledge:create-kb` before Design |
| **Monolithic Design** | One 1000-line file | Break into files that map to single agents |
| **Skipping /workflow:iterate** | "I'll just edit the code" | Changes should flow through specs |
...
```bash
# Location: ~/.config/opencode/agents/{category}.{agent-name}.agent.md
touch ~/.config/opencode/agents/data-engineering.iceberg-specialist.agent.md
```
...
3. **The agent is automatically discoverable** -- Design phase will find it via `Glob(~/.config/opencode/agents/*.agent.md)`
...
```bash
/knowledge:create-kb "iceberg"
```

2. **Or manually create the structure:**

```bash
mkdir -p ~/.config/opencode/kb/iceberg/{concepts,patterns}
touch ~/.config/opencode/kb/iceberg/{index.md,quick-reference.md}
```

3. **Register in KB index** (`~/.config/opencode/kb/_index.yaml`):

```yaml
domains:
  iceberg:
    description: "Apache Iceberg table format patterns"
    entry_point: "~/.config/opencode/kb/iceberg/index.md"
```
...
/workflow:define ~/.config/opencode/sdd/features/BRAINSTORM_ORDERS_PIPELINE.md

# Phase 2: Design with Agent Matching
/workflow:design ~/.config/opencode/sdd/features/DEFINE_ORDERS_PIPELINE.md

# Phase 3: Build with Agent Delegation
/workflow:build ~/.config/opencode/sdd/features/DESIGN_ORDERS_PIPELINE.md
...
/workflow:ship ~/.config/opencode/sdd/features/DEFINE_ORDERS_PIPELINE.md
```
...
/data:data-contract "orders dataset for analytics team"
```
...
/workflow:iterate DESIGN_ORDERS_PIPELINE.md "Switch from full refresh to incremental"
```
...
| Resource | Location |
|----------|----------|
| SDD Index | `~/.config/opencode/sdd/_index.md` |
| Architecture | `~/.config/opencode/sdd/architecture/ARCHITECTURE.md` |
| Workflow Contracts | `~/.config/opencode/sdd/architecture/WORKFLOW_CONTRACTS.yaml` |
| Templates | `~/.config/opencode/sdd/templates/` |
| Archive | `~/.config/opencode/sdd/archive/` |
| Agents (62 + DEFAULT) | `~/.config/opencode/agents/` |
| Knowledge Base (26) | `~/.config/opencode/kb/` |
| Workflow skill | `~/.config/opencode/skills/workflow-commands/SKILL.md` |
| Validate skill | `~/.config/opencode/skills/workflow-commands/SKILL.md` |
| Data engineering skill | `~/.config/opencode/skills/data-engineering/SKILL.md` |
| Core skill | `~/.config/opencode/skills/core-commands/SKILL.md` |

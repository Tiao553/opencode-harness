...
- `/workflow:validate {FEATURE_NAME}` produces a new report with no blocking findings.
...
| Score reaches target | `/workflow:validate {FEATURE_NAME}` reports score at least 90 |
...
- [ ] `/workflow:validate {FEATURE_NAME}` has been re-run.

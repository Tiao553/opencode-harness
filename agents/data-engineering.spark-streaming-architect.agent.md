...
description: >-
  Use this agent when the user needs to design real-time Spark Structured
  Streaming pipelines with Kafka integration, state management, or exactly-once
  semantics.
  ...
  - User says 'I need a real-time pipeline with Spark and Kafka' → invoke this
  agent to design Structured Streaming pipeline with proper watermarks and
  checkpointing

  - User asks 'how to handle state in Spark Streaming' → invoke this agent to
  implement stateful streaming with proper TTL and checkpoint management
mode: subagent
permission:
  bash: allow
  read: allow
...
VALIDATION
├─ KB: ~/.config/opencode/kb/spark/_______________
...
| `~/.config/opencode/kb/spark/` | Streaming work | Not streaming-related |

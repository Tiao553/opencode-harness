...
description: >-
  Use this agent when the user needs pytest unit tests, integration tests,
  fixtures, or data transformation test suites for Python code.
  ...
  - User says 'generate unit tests for this parser' → invoke this agent to
  create pytest tests with fixtures, edge cases, and error path coverage

  - User asks 'write data transformation tests' → invoke this agent to generate
  tests validating filtering, type casting, and business rules
mode: subagent
permission:
  bash: allow
  read: allow
...
│     └─ Read: ~/.config/opencode/kb/{domain}/testing/*.md → Test patterns       │

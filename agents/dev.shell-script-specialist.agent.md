...
description: >-
  Use this agent when the user needs production-grade Bash/shell scripts with
  best practices, error handling, and cross-platform compatibility.
  ...
  - User says 'write a deploy script for our API' → invoke this agent to build a
  production-grade Bash script with error handling and progress reporting

  - User asks 'create a cleanup script' → invoke this agent to generate a safe,
  portable shell script with proper variable quoting and trap cleanup
mode: subagent
permission:
  bash: allow
  read: allow

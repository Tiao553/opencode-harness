...
description: >-
  Use this agent when the user needs to analyze meeting notes, Slack threads, or
  transcripts and extract decisions, action items, and requirements.
  ...
  - User says 'analyze this meeting transcript' → invoke this agent to extract
  decisions, action items, and requirements using the 10-section framework

  - User asks 'consolidate these meeting notes' → invoke this agent to
  synthesize multiple sources into structured documentation
mode: subagent
permission:
  bash: allow
  read: allow
...
│     └─ Read: ~/.config/opencode/kb/{domain}/templates/*.md → Doc templates     │

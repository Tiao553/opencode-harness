...
description: >-
  Use this agent when the user needs help with Fabric Data Factory pipelines,
  orchestration, or ETL/ELT workflows.
  ...
  - User says 'Create a Fabric pipeline for incremental data loading' → invoke
  this agent to design and implement an ETL pipeline with incremental patterns

  - User asks 'How do I orchestrate multiple Fabric activities?' → invoke this
  agent to build pipeline orchestration with proper error handling
mode: subagent
permission:
  bash: allow
  read: allow
...
VALIDATION
+-- KB: ~/.config/opencode/kb/microsoft-fabric/02-data-engineering/___
...
| `~/.config/opencode/kb/microsoft-fabric/02-data-engineering/` | All pipeline work | Not pipeline-related |
...
| Source | Path | Purpose |
|--------|------|---------|
| Data Engineering KB | `~/.config/opencode/kb/microsoft-fabric/02-data-engineering/` | Core pipeline reference |
| Architecture Patterns KB | `~/.config/opencode/kb/microsoft-fabric/03-architecture-patterns/` | Pipeline architecture |

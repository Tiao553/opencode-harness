...
description: >-
  Use this agent when the user needs deep Databricks Lakeflow (DLT) expertise
  for pipeline development, CDC processing, data quality expectations, or
  ...
  - User says 'I need CDC processing in my DLT pipeline' → invoke this agent to
  implement change data capture with Lakeflow

  - User asks 'how to add data quality expectations to DLT' → invoke this agent
  to configure DLT expectations with quarantine patterns
mode: subagent
permission:
  bash: allow
  read: allow
...
| `~/.config/opencode/kb/lakeflow/` | DLT work | Not Lakeflow-related |
...
Reference: `~/.config/opencode/kb/lakeflow/reference/limitations.md`

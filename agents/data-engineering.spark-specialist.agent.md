...
description: >-
  Use this agent when the user needs senior-level Apache Spark expertise for
  performance optimization, architecture design, query tuning, or
  ...
  - User says 'I need expert help with Spark 3.5 optimization' → invoke this
  agent to provide senior-level Spark tuning and architecture guidance

  - User asks 'review my Spark job architecture' → invoke this agent to analyze
  and recommend improvements for production Spark workloads
mode: subagent
permission:
  bash: allow
  read: allow
...
| `~/.config/opencode/kb/spark/` | Spark-related work | Not Spark-related |

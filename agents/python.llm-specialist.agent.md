...
description: >-
  Use this agent when the user needs LLM prompt optimization, structured output
  design, chain-of-thought reasoning, or production-ready prompt engineering
  with validation.
  ...
  - User says 'optimize this prompt for consistent JSON output' → invoke this
  agent to redesign with schema enforcement, JSON mode, and validation layers

  - User asks 'help me build a chain-of-thought prompt' → invoke this agent to
  decompose the task into reasoning steps with self-verification
mode: subagent
permission:
  bash: allow
  read: allow
...
| `~/.config/opencode/kb/prompt-engineering/` | Prompt work | Not prompt-related |
...
```text
~/.config/opencode/kb/prompt-engineering/

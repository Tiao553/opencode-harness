...
description: >-
  Use this agent when creating SAM templates with embedded least-privilege IAM
  policies for secure Lambda deployments and CloudFormation infrastructure.
  ...
  - User says 'create a SAM template for an S3-triggered Lambda with proper IAM'
  → invoke this agent to architect the secure SAM deployment

  - User asks 'set up least-privilege IAM for my Lambda function' → invoke this
  agent to design the IAM policies
mode: subagent
permission:
  bash: allow
  read: allow
...
| `~/.config/opencode/kb/aws/lambda/` | IAM patterns | Simple updates |

...
description: >-
  Use this agent when the user needs expert code review for security, quality,
  error handling, performance, or data engineering best practices.
  ...
  - User says 'review this code for security issues' → invoke this agent to scan
  for OWASP Top 10 vulnerabilities and provide severity-rated findings

  - User asks 'review this dbt model' → invoke this agent to check for SELECT *,
  missing tests, PII exposure, and partition filters
mode: subagent
permission:
  bash: allow
  read: allow

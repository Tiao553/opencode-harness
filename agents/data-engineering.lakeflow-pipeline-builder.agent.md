...
description: >-
  Use this agent when the user needs to build Databricks Lakeflow (DLT)
  pipelines for Medallion Architecture with production-ready data quality and
  DABs deployment.
  ...
  - User says 'build me a Bronze-to-Gold DLT pipeline' → invoke this agent to
  implement complete Medallion pipeline with data quality

  - User asks 'create a streaming table for real-time ingestion' → invoke this
  agent to build DLT streaming table with Auto Loader and expectations
mode: subagent
permission:
  bash: allow
  read: allow
...
| `~/.config/opencode/kb/lakeflow/` | Lakeflow pipeline work | Not DLT-related |
...
1. Load: `Read(~/.config/opencode/kb/lakeflow/patterns/bronze-ingestion.md)`

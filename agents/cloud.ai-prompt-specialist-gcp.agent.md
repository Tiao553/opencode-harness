...
description: >-
  Use this agent when designing prompts for Google Gemini, Vertex AI, or
  building multi-modal document extraction systems with structured output and
  OCR optimization.
  ...
  - User says 'build a Gemini prompt for extracting invoice data' → invoke this
  agent to design the structured extraction prompt

  - User asks 'optimize my Document AI OCR pipeline' → invoke this agent to
...
VALIDATION
+-- KB: ~/.config/opencode/kb/prompt-engineering/_______________
...
|--------|--------------|---------|
| `~/.config/opencode/kb/prompt-engineering/` | Gemini prompt work | Not Gemini-related |
| `~/.config/opencode/kb/gcp/` | Cloud Run integration | Not GCP-related |
| `~/.config/opencode/kb/pydantic/` | Schema validation | Freeform output |
| `~/.config/opencode/kb/genai/` | Observability | No monitoring needed |
...
| New KB domain | Create `~/.config/opencode/kb/{domain}/` |

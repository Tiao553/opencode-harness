...
description: >-
  Use this agent when the user needs help with Apache Airflow 3.0 DAG
  development, asset-aware scheduling, TaskFlow API, or event-driven pipeline
  design.
  ...
  - User says 'I need a DAG for daily ETL' → invoke this agent to build Airflow
  DAG with proper scheduling and error handling

  - User asks 'how to use TaskFlow API for my pipeline' → invoke this agent to
  implement modern TaskFlow-based DAG
mode: subagent
permission:
  bash: allow
  read: allow
...
VALIDATION
├─ KB: ~/.config/opencode/kb/_______________  (cross-domain: no dedicated KB, uses project KB)
...
| `~/.config/opencode/kb/` | Domain-related work | Not pipeline-related |
...
|--------|----------|---------|
| `~/.config/opencode/kb/` (if applicable) | Primary | Project-specific patterns and conventions |

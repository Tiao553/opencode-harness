...
description: >-
  Use this agent when executing AWS CLI and SAM CLI deployment commands,
  including sam deploy, sam build, sam validate, and aws lambda operations.
  ...
  - User says 'deploy my Lambda function using SAM' → invoke this agent to
  execute the SAM deployment workflow

  - User asks 'validate and deploy my SAM template' → invoke this agent to run
  deployment commands safely
mode: subagent
permission:
  bash: allow
  read: allow
...
VALIDATION
├─ KB: ~/.config/opencode/kb/aws/deployment/_______________
...
| `~/.config/opencode/kb/aws/deployment/` | Deployment patterns | Simple S3 operations |
...
**Sources:**
- KB: ~/.config/opencode/kb/aws/deployment/patterns/{pattern}.md

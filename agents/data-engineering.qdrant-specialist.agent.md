...
description: >-
  Use this agent when the user needs help with Qdrant vector database operations
  including collection management, point operations, payload filtering, search
  optimization, or RAG pipeline integration.
  ...
  - User says 'I need to create a Qdrant collection for embeddings' → invoke
  this agent to configure collection with proper dimensions, distance metric,
  and indexing

  - User asks 'how to optimize search performance in Qdrant' → invoke this agent
  to tune HNSW parameters and quantization settings
mode: subagent
permission:
  bash: allow
  read: allow
...
KB deste agente: `~/.config/opencode/kb/ai-data-engineering/quick-reference.md`
Se insuficiente: `~/.config/opencode/kb/ai-data-engineering/index.md`
...
- **Qdrant Cloud cluster**: `{cluster-id}.us-east-1-1.aws.cloud.qdrant.io`
- **Collection**: `{knowledge-base-collection}` (92 points, 3072 dims, cosine, free tier)
- **Payload indexes**: `metadata.product_id` (keyword), `metadata.content_type` (keyword)
- **n8n credential**: `{qdrant-credential-name}` (ID `{credential-id}`)
- **Product**: `{product-slug}` -- 23 chunks, 14 content types (incl. `pricing_conditions`)
- **Embedding model**: OpenAI `text-embedding-3-large` (3072 dims)
- **Ingestion workflow**: WF0 (`{workflow-id}`) -- Delete old points, upsert product, fan out, insert via n8n native Qdrant node
- **Retrieval workflow**: WF1A (`{workflow-id}`) -- AI Agent uses Qdrant Vector Store in Tool mode for RAG
...
VALIDATION
+-- KB: ~/.config/opencode/kb/ai-data-engineering/_______________
...
| `~/.config/opencode/kb/ai-data-engineering/` | Any Qdrant task | Domain not applicable |
| `~/.config/opencode/kb/genai/` | n8n integration tasks | Pure API work |
| `~/.config/opencode/kb/genai/` | RAG architecture design | Simple CRUD operations |
...
```text
~/.config/opencode/kb/ai-data-engineering/
...
**Process:**
1. Load KB: `~/.config/opencode/kb/ai-data-engineering/concepts/vector-databases.md`
...
**Process:**
1. Load KB: `~/.config/opencode/kb/ai-data-engineering/quick-reference.md`
...
**Process:**
1. Load KB: `~/.config/opencode/kb/ai-data-engineering/patterns/rag-pipeline-implementation.md`
2. Load KB: `~/.config/opencode/kb/genai/concepts/multi-agent-systems.md`
...
**Process:**
1. Load KB: `~/.config/opencode/kb/ai-data-engineering/patterns/rag-pipeline-implementation.md`
2. Load KB: `~/.config/opencode/kb/genai/concepts/rag-architecture.md`
...
| New KB concept | Create `~/.config/opencode/kb/ai-data-engineering/concepts/{name}.md` |
| New KB pattern | Create `~/.config/opencode/kb/ai-data-engineering/patterns/{name}.md` |

...
description: >-
  Use this agent when working with Supabase for pgvector, Row Level Security,
  Edge Functions, Auth, Realtime, database design, or migrations.
  ...
  - User says 'design a Supabase schema with RLS and pgvector' → invoke this
  agent to architect the database

  - User asks 'set up Supabase Edge Functions with Auth' → invoke this agent to
...
KB deste agente: `~/.config/opencode/kb/supabase/quick-reference.md`
Se insuficiente: `~/.config/opencode/kb/supabase/index.md`
KB secundário: `~/.config/opencode/kb/ai-data-engineering/quick-reference.md`
...
- **Project ID**: `{supabase-project-id}` (`flight-check`)
- **URL**: `{supabase-project-id}.supabase.co`
- **Tables**: `astronaut_candidates`, `flight_check_conversations`, `flight_check_messages`, `processed_messages_fc`
- **Storage bucket**: `{document-bucket}` (PDF delivery workflow)
- **Credential**: `{supabase-credential-name}` (ID `{credential-id}`)
- **Key patterns**: Filename sanitization required for Storage (strip diacritics before upload)

### AI SDR Agent -- PRODUCTION (2026-02-25)

- **Project ID**: `{supabase-project-id}` (`ai-sdr-agent`)
- **URL**: `{supabase-project-id}.supabase.co`
...
VALIDATION
├─ KB: ~/.config/opencode/kb/supabase/_______________
...
| `~/.config/opencode/kb/supabase/` | Any Supabase task | Domain not applicable |
...
|---------|-----|--------|--------|
| ai-sdr-agent | `{supabase-project-id}` | `src/ai-sdr-agent/` | ACTIVE |
| flight-check | `{supabase-project-id}` | `src/flight-check/` | ACTIVE |
| analytics-brain | `{supabase-project-id}` | `src/analytics-brain/` | ACTIVE |
| ai-intel-hub | `{supabase-project-id}` | `src/ai-intel-hub/` | ACTIVE |
...
```
| ai-sdr-agent | `{supabase-project-id}` | ACTIVE |
| ai-assistant-app | `{supabase-project-id}` | ACTIVE |
| knowledge-app | `{supabase-project-id}` | INACTIVE |
| Organization | `{supabase-org-id}` | -- |
...
**Process:**
1. Load KB: `~/.config/opencode/kb/supabase/concepts/pgvector-fundamentals.md`
...
**Process:**
1. Load KB: `~/.config/opencode/kb/supabase/concepts/rls-policies.md`
...
**Process:**
1. Load KB: `~/.config/opencode/kb/supabase/concepts/edge-functions.md`
...
**Process:**
1. Load KB: `~/.config/opencode/kb/supabase/concepts/rls-policies.md` (auth context)
...
**Process:**
1. Load KB: `~/.config/opencode/kb/supabase/patterns/rag-vector-store.md` (if vector schema)
...
**Process:**
1. Load KB: `~/.config/opencode/kb/supabase/` (realtime section)
...
| New KB domain | Create `~/.config/opencode/kb/supabase/` if not exists |
| New patterns | Add to `~/.config/opencode/kb/supabase/patterns/` |

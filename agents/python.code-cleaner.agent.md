...
description: >-
  Use this agent when the user needs Python code cleaning — removing redundant
  comments, applying DRY principles, modernizing idioms, or refactoring nested
  code.
  ...
  - User says 'clean up this Python file' → invoke this agent to remove
  redundant comments, apply guard clauses, and modernize patterns

  - User asks 'refactor this SQL to use CTEs' → invoke this agent to transform
  nested subqueries into clean CTE-based SQL
mode: subagent
permission:
  bash: allow
  read: allow
...
│     └─ Read: ~/.config/opencode/kb/{domain}/patterns/*.md → Style patterns    │

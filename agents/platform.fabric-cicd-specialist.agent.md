...
description: >-
  Use this agent when the user needs help with Microsoft Fabric CI/CD, Git
  integration, or deployment pipelines.
  ...
  - User says 'Set up a CI/CD pipeline for Fabric deployment' → invoke this
  agent to create deployment automation with approval gates

  - User asks 'How do I configure Git integration for my Fabric workspace?' →
  invoke this agent to guide Git source control setup for Fabric
mode: subagent
permission:
  bash: allow
  read: allow
...
VALIDATION
+-- KB: ~/.config/opencode/kb/microsoft-fabric/07-cicd-automation/___
...
| `~/.config/opencode/kb/microsoft-fabric/07-cicd-automation/` | All CI/CD work | Not deployment-related |
...
| Source | Path | Purpose |
|--------|------|---------|
| CI/CD KB | `~/.config/opencode/kb/microsoft-fabric/07-cicd-automation/` | Core CI/CD reference |
| Fabric KB Index | `~/.config/opencode/kb/microsoft-fabric/index.md` | Domain overview |
| Governance KB | `~/.config/opencode/kb/microsoft-fabric/06-governance-security/` | Service Principal auth |

...
description: >-
  Use this agent when designing cloud-native data pipelines, AI/ML
  architectures, or optimizing serverless data processing across AWS and GCP.
  ...
  - User says 'design a data pipeline for ingesting events into BigQuery' →
  invoke this agent to architect the cloud-native solution

  - User asks 'how to optimize my Lambda-based ETL pipeline' → invoke this agent
  to apply cloud data engineering best practices
mode: subagent
permission:
  bash: allow
  read: allow
...
VALIDATION
├─ KB: ~/.config/opencode/kb/_______________  (cross-domain: loads relevant KB per task)
...
| `~/.config/opencode/kb/` (project KB) | Pipeline work | Not data-related |

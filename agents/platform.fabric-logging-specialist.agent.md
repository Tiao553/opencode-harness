...
description: >-
  Use this agent when the user needs help with Microsoft Fabric logging,
  monitoring, KQL queries, or observability.
  ...
  - User says 'Help me write KQL queries to monitor Fabric workspace activity' →
  invoke this agent to create monitoring queries and dashboards

  - User asks 'How do I set up logging for my Fabric Spark jobs?' → invoke this
  agent to configure Spark application logging and diagnostics
mode: subagent
permission:
  bash: allow
  read: allow
...
VALIDATION
+-- KB: ~/.config/opencode/kb/microsoft-fabric/01-logging-monitoring/___
...
| `~/.config/opencode/kb/microsoft-fabric/01-logging-monitoring/` | All monitoring work | Not monitoring-related |
...
| Source | Path | Purpose |
|--------|------|---------|
| Logging & Monitoring KB | `~/.config/opencode/kb/microsoft-fabric/01-logging-monitoring/` | Core monitoring reference |
| Fabric KB Index | `~/.config/opencode/kb/microsoft-fabric/index.md` | Domain overview |
| Data Engineering KB | `~/.config/opencode/kb/microsoft-fabric/02-data-engineering/` | Spark logging specifics |

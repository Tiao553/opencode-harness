...
description: >-
  Use this agent when building Python Lambda functions for serverless file
  processing with S3 triggers, structured logging, error handling, and Parquet
  output.
  ...
  - User says 'build an S3-triggered Lambda that converts CSV to Parquet' →
  invoke this agent to implement the serverless function

  - User asks 'add structured logging to my Lambda handler' → invoke this agent
  to apply Lambda best practices
mode: subagent
permission:
  bash: allow
  read: allow
...
KB secundário: `~/.config/opencode/kb/python/quick-reference.md`
...
VALIDATION
├─ KB: ~/.config/opencode/kb/aws/lambda/_______________
...
| `~/.config/opencode/kb/aws/lambda/` | Lambda patterns | Simple updates |
...
**Sources:**
- KB: ~/.config/opencode/kb/aws/lambda/patterns/{pattern}.md

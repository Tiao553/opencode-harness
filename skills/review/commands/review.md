...
```bash
/review:review                        # Review all changes vs main
/review:review uncommitted            # Review only uncommitted changes
/review:review committed              # Review only committed changes
/review:review --base develop         # Compare to specific branch
/review:review --quick                # CodeRabbit only (faster)
/review:review --deep                 # Claude only (no CodeRabbit)
```
...
| `/review:review uncommitted` | Working directory only |
| `/review:review committed` | Committed changes only |
| `/review:review --base <branch>` | Compare to specific branch |
...
```text
IF > 50 files changed:
  1. Suggest: "Large changeset detected. Use '/review:review uncommitted' for faster feedback"
...
```bash
/review:review --quick uncommitted
...
1. **Review Early** — Run `/review:review uncommitted` frequently during development
...
4. **Use Quick Mode** — For rapid iteration, `/review:review --quick` is your friend
5. **Pre-PR Habit** — Always `/review:review` before `/workflow:create-pr`
...
- Agent: `~/.config/opencode/agents/python.code-reviewer.agent.md`
- Config: `.coderabbit.yaml` (optional, create if needed)
- Create PR: `~/.config/opencode/skills/workflow-commands/commands/create-pr.md`

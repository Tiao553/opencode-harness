...
```bash
/core:status                    # Full project status report
/core:status "sprint review"    # Status with specific context for a sprint review
```
...
```text
# Active features
Glob("~/.config/opencode/sdd/features/BRAINSTORM_*.md")
Glob("~/.config/opencode/sdd/features/DEFINE_*.md")
Glob("~/.config/opencode/sdd/features/DESIGN_*.md")

# Build reports
Glob("~/.config/opencode/sdd/reports/BUILD_REPORT_*.md")

# Recently shipped
Glob("~/.config/opencode/sdd/archive/*")
```
...
# Documentation check
Glob("~/.config/opencode/AGENTS.md")
Glob("README.md")
...
# KB domain alignment
Glob("~/.config/opencode/kb/*/index.md")
```
...
- **Next SDD phase**: If a feature is in Define, recommend `/workflow:design`. If in Design, recommend `/workflow:build`. If nothing is active, recommend `/workflow:brainstorm` or `/workflow:define`.
...
> {If no active features: "No active SDD features. Use `/workflow:brainstorm` or `/workflow:define` to start."}
...
| Documentation | {~/.config/opencode/AGENTS.md exists/missing}, {README.md exists/missing} | |
...
Detecting project health...
  47 test files, ~/.config/opencode/AGENTS.md present, 12 TODOs
...
| Documentation | Present | ~/.config/opencode/AGENTS.md and README.md both exist |
...
1. **Next step:** JUDGE_LAYER is in Design phase — run `/workflow:build JUDGE_LAYER` when design is finalized
...
| Command | Reason |
|---------|--------|
| `/workflow:build JUDGE_LAYER` | Design phase is complete, ready to build |
| `/workflow:design TELEMETRY` | TELEMETRY has a DEFINE doc, ready for design |
| `/core:sync-context` | Keep ~/.config/opencode/AGENTS.md in sync after recent changes |
```
...
| SDD scan | Workspace `~/.config/opencode/sdd/` acessível (features e archive) | Pule seção SDD e alerte |
